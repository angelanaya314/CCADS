...
# Ruta del archivo
filesheet = "BD_DocTour.xlsx"

# Creación del objeto load_workbook
wb = load_workbook(filesheet)
ws_comisiones = wb["1_Comisiones"]
data_comisiones = ws_comisiones.values
columnas_comisiones = next(data_comisiones)[0:]
comisiones314 = pd.DataFrame(data_comisiones, columns = columnas_comisiones)
index_comisiones314 = comisiones314.iloc[:, 0]
comisiones314.set_index(index_comisiones314,inplace = True)
comisiones314 = comisiones314.iloc[:, 1:]
...
option_selected = option_menu(
	menu_title=None,
	options=["Planeación financiera", "Escenarios", "Dashboard"],
...
if option_selected == "Planeación financiera":
    ...
    sidebar.header('Planeación financiera \n `Métricas y escenarios`')
...
#-----------------------------------------------------------------------------------------------------------------------------#
    ...
    columna3.line_chart(df_un_inflacion_1)
    st.write(" --- ")
    ...
    columna3.line_chart(df_un_inflacion_2)
    st.write(" --- ")
    ...
    columna3.line_chart(df_un_inflacion_3)
    st.write(" --- ")
    ...
    columna3.line_chart(df_utilidades_escenarios)
    st.write(" --- ")
#-----------------------------------------------------------------------------------------------------------------------------#
elif option_selected == "Dashboard": 
    df_dash= pd.read_excel("BD_DocTour.xlsx",sheet_name="3_Membresías")   
      
    ab1, ab2 = st.columns((3,2))
    fig = px.bar(df_dash, x="Membresias", y="Cantidad ", title="Numero de usuarios por  membresías",color=["blue", "black", "silver"],color_discrete_map="identity")
    ab1.plotly_chart(fig)

    values=[65,35,100]
    labels=["Básica","Black","Platino"]
    fig2 = px.pie(values=values, names=labels, color=labels, title="Gráfica de pastel del número de membresías",color_discrete_map={"Básica":"blue","Black":"black","Platino":"gray"})
    ...
    fig = px.line(df_Margen,x="Tiempo" ,y="Margenes", title="Tiempo vs Margenes",markers=True, text="Margenes")
    ...
    y=[10, 11, 12, 13, 14],
    marker_color=colors)])
    fig3.update_layout(title_text="Mes con menor margen")
    ...
    fig = px.line(df_Utilidad ,x="Tiempo" ,y="Utilidad", title="Tiempo vs Utilidad",markers=True, text="Utilidad")
    fig.update_traces(textposition="bottom right")
    ...
    ad2.markdown("<h1 style='text-align: center; color: darkblue; font-size: 4.4rem;'>Junio</h1>", unsafe_allow_html=True)
    ...
    ad3.markdown("<h1 style='text-align: center; color: darkblue; font-size: 4.4rem;'>6000</h1>", unsafe_allow_html=True)
    ...
    fig = px.bar(df_Porcentaje, x="% de utilización", y="Servicios", title="Porcentaje de utilización de cada servicio",
    hover_data=['% de utilización', 'Servicios'], color='% de utilización')
    fig.update_yaxes(tickformat=".2%")
    ...
    ae2.markdown("<h1 style='text-align: center; color: darkblue; font-size: 4.4rem;'>Medicina general</h1>", unsafe_allow_html=True)
    ...
    ae3.markdown("<h1 style='text-align: center; color: darkblue; font-size: 4.4rem;'>25%</h1>", unsafe_allow_html=True)                                
